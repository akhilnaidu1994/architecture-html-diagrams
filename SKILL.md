...
description: Create polished dark-themed standalone HTML pages with inline SVG architecture diagrams. Use when Codex needs to produce system architecture diagrams, cloud architecture diagrams, AWS/GCP/Azure infrastructure maps, microservice diagrams, AI/backend architecture maps, security flow diagrams, network topology diagrams, data-flow diagrams, or complex component relationship diagrams with labeled arrows, grouped boundaries, legends, and a professional dark technical visual style.
---

# Architecture HTML Diagrams

Create a single self-contained `.html` file containing a dark, grid-backed SVG architecture diagram. The output should be visually polished enough for docs, proposals, design reviews, screenshots, and management reviews.
...
1. Parse the user's request into:
   ...
   - Flows: request paths, async events, data writes, control-plane operations, auth/security paths.
...
- Include a concise title, subtitle, main diagram, legend, 3-4 summary cards, and a small footer.
